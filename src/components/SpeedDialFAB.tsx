...
export function SpeedDialFAB({ onAction, reducedMode = false }: SpeedDialFABProps) {
  ...
  const handleSelect = (action: SpeedDialAction) => { setOpen(false); onAction(action); };
  const bottomPosition = isMobile ? "bottom-[4.5rem]" : "bottom-6";

  return (
    <>
      {open && (
        <div className="fixed inset-0 bg-background/60 backdrop-blur-sm z-40 animate-fade-in" onClick={() => setOpen(false)} />
      )}
            ...
            {actions.map((a, i) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-3 pl-4 pr-3 py-2.5 rounded-full shadow-lg ${a.color} min-h-[44px] hover:scale-105 active:scale-95 transition-all duration-200`}
                  style={{ animationDelay: `${i * 40}ms`, animation: "slideUp 200ms ease-out both" }}
                >
                  <span className="font-mono text-xs font-medium tracking-wider whitespace-nowrap">{a.label}</span>
