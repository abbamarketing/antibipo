...
export function UnifiedKanban({ energy, lastMoodValue, preferredModule = null }: UnifiedKanbanProps) {
  ...
  const handleDelegate = (id: string) => { updateTask(id, { status: "aguardando" }); logActivity("tarefa_delegada", { task_id: id, hora: brasiliaTimeString() }); };
  const handlePush = (id: string) => { const task = state.tasks.find((t) => t.id === id); updateTask(id, { urgencia: Math.max(1, (task?.urgencia || 2) - 1) as 1 | 2 | 3 }); logActivity("tarefa_empurrada", { task_id: id, hora: brasiliaTimeString() }); };
  ...
  const handleMoveStatus = (id: string, newStatus: string) => { updateTask(id, { status: newStatus as any }); logActivity("tarefa_movida", { task_id: id, status: newStatus, hora: brasiliaTimeString() }); };
      ...
      <div className="space-y-4 animate-fade-in">
        ...
        <div>
          ...
          <div className="flex items-center gap-3 mt-2">
            <span className="text-[10px] font-mono text-muted-foreground">{filtered.filter((i) => i.status === "hoje").length} hoje</span>
            <span className="text-muted-foreground/30">·</span>
            <span className="text-[10px] font-mono text-muted-foreground">{filtered.length} total</span>
            <span className="text-muted-foreground/30">·</span>
            <span className="text-[10px] font-mono text-muted-foreground">{completedToday.length} feitas</span>
          </div>
        </div>

        {/* Module filter */}
        <div className="flex gap-1.5">
          <button onClick={() => setFilterModule(null)} className={`px-2.5 py-1 rounded-md text-[10px] font-mono transition-all ${!filterModule ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground"}`}>TODOS</button>
          {(["trabalho", "casa", "saude"] as const).map((m) => {
            const Icon = MODULE_ICONS[m];
            return (
              <button key={m} onClick={() => setFilterModule(filterModule === m ? null : m)} className={`flex items-center gap-1 px-2.5 py-1 rounded-md text-[10px] font-mono transition-all ${filterModule === m ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground"}`}>
                <Icon className="w-3 h-3" />{MODULE_LABELS[m]}{moduleCounts[m] > 0 && <span className="opacity-60">({moduleCounts[m]})</span>}
              </button>
            );
          })}
        </div>
